...
router.post('/upload', async ctx => {
  ...
  const { name, hash } = ctx.request.fields
  ...
  let filePath = path.resolve(__dirname, './static/upload/' + name.split('-')[0])
  ...
  fs.rename(file.path, path.resolve(__dirname, filePath, name), () => { });
